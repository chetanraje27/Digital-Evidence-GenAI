...
def save_vae_variations(output_dir: Path, device: torch.device, source: Path) -> None:
    ...
    axes[0, 1].imshow(deterministic)
    axes[0, 1].set_title("Deterministic\nz = μ")
    for index, variation in enumerate(variations, 2):
        axes[0, index].imshow(variation["image"])
        axes[0, index].set_title(
            f"Stochastic {index - 1}\nmean |Δ| {variation['mean_abs_delta_from_deterministic']:.5f}"
        )
        axes[1, index].imshow(variation["difference_map"], cmap="magma")
        axes[1, index].set_title(
            f"Absolute difference {index - 1}\nmax {variation['difference_map_max']:.5f}"
        )
    axes[1, 0].text(0.5, 0.5, "Difference maps use\nper-sample measured scales", ha="center", va="center")
    axes[1, 1].text(0.5, 0.5, "Images are not\nartificially modified", ha="center", va="center")
...
def save_text_demo(output_dir: Path, device: torch.device) -> dict[str, object]:
    ...
    content = (
        ...
        "## Generated output\n\n"
        f"{result.output}\n\n"
        "The summary is generated by the pretrained Transformer. Structured entity and observation "
        "indicators are disclosed source-text pattern matches used to reduce unsupported invention.\n"
    )
